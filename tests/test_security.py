...
def test_is_safe_url_with_non_https():
    ...
    assert is_safe_url("http://bsky.social/path") is False
...
def test_is_safe_url_with_invalid_urls():
    ...
    assert is_safe_url("ftp://bsky.social/path") is False


def test_is_safe_url_with_ip_addresses():
    """Test that IP addresses are considered unsafe."""
    assert is_safe_url("https://127.0.0.1/path") is False
    assert is_safe_url("https://192.168.1.1/path") is False
    assert is_safe_url("https://[::1]/path") is False
